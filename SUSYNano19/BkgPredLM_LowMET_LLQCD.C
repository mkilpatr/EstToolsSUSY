...
void BkgPredLM_LowMET_LLQCD(){
  ...
  vector<std::string> bkgnames  = {"qcd_pred", "ttbarplusw_pred"};
  vector<TString> bkglabels = {"QCD", "t#bar{t}/W"};
...
//    leg->SetNColumns(2);
  ...
  cout << "\n Summary Traditional \n";
  s.printSummary({l.yields.at("_pred"), q.yields.at("_pred")}, s.yields.at("data-sr"));
