...
void SystLHEScaleWeight_LL(){
  ...
  std::string outfile_path = "values_unc_" + bins + mc + (string)binMap_ + "_ll_lhe.conf";
  ...
  // lhe - up
  {
    sys_name = "LHEScale_Up";
    lhewgt = "LHEScaleWeight_Up";
    auto llb = getLLBPred();
    for (auto &p : llb) proc_syst_pred[p.first][sys_name] = p.second;
  }
  ...
  // lhe - down
  {
    sys_name = "LHEScale_Down";
    lhewgt = "LHEScaleWeight_Down";
    auto llb = getLLBPred();
    for (auto &p : llb) proc_syst_pred[p.first][sys_name] = p.second;
  }
