...
void SystPowMG_LL(){
  ...
  std::string outfile_path = "values_unc_" + bins + mc + (string)binMap_+ "_ll_mgpow.conf";
  ...
  {
    sys_name = "nominal";
    topptwgt = "Stop0l_topMGPowWeight*Stop0l_topptOnly";
    auto llb = getLLBPred();
  ...
  // btag - up
  {
    sys_name = "PowhegOverMG_Up";
    topptwgt = "Stop0l_topptOnly";
    auto llb = getLLBPred();
    for (auto &p : llb) proc_syst_pred[p.first][sys_name] = p.second;
  }
  ...
  for (auto &bkg : bkgnames){
    ...
    for (auto &sPair : proc_syst_pred[bkg]){
      ...
      if(sPair.first.EndsWith("_Up")){
        ...
        uncs = Quantity::combineUpDownSepUncs(varup);
	uncs_Up = uncs.first;
