...
void SystTopPT_LL(){
  ...
  std::string outfile_path = "values_unc_" + bins + mc + (string)binMap_+ "_ll_toppt.conf";
  ...
  {
    sys_name = "toppt_Up";
    topptwgt = "Stop0l_topMGPowWeight*Stop0l_topptOnly_Up";
    auto llb = getLLBPred();
    for (auto &p : llb) proc_syst_pred[p.first][sys_name] = p.second;
  }
  ...
  {
    sys_name = "toppt_Down";
    topptwgt = "Stop0l_topMGPowWeight*Stop0l_topptOnly_Down";
    auto llb = getLLBPred();
    for (auto &p : llb) proc_syst_pred[p.first][sys_name] = p.second;
  }
