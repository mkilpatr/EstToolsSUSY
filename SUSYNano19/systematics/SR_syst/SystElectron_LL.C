...
map<TString, vector<Quantity>> getLLBPred(){
  ...
  if (EstTools::doLepSyst == true){
         if(EstTools::region.Contains("2016and2017")) l.pred2016and2017lep();
    else if(EstTools::region.Contains("201"))         l.predYearlep();
    else				              l.predlep();
  } else{
...
void SystElectron_LL(){
  ...
  std::string outfile_path = "values_unc_" + bins + mc + "_ll_electron.conf";
