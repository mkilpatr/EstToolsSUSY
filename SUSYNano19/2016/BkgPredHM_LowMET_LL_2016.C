...
void BkgPredHM_LowMET_LL_2016(){
  ...
  auto convert = [&bkgnames](const ToyCombination &c, vector<TH1*> &hists, vector<TGraphAsymmErrors*> &graphs){
    for(const auto &s : bkgnames){
      hists.push_back(convertToHist(c.getPrediction(s), s, ";High #Deltam Validation Region;Events"));
      graphs.push_back(convertToGraphAsymmErrors(c.getPrediction(s), s+"_gr", ";High #Deltam Validation Region;Events"));
    }
    graphs.push_back(convertToGraphAsymmErrors(c.getPrediction(), "pred_total_gr", ";High #Deltam Validation Region;Events"));
  };
  ...
  vector<TH1*> mc;
  mc.push_back(convertToHist(l.yields.at("ttbarplusw-sr"),"ttbarplusw_mc",";High #Deltam Validation Region;Events"));

  auto sigcfg = sigConfig();
  BaseEstimator s(sigcfg);
  s.calcYields();

  auto hdata = convertToHist(s.yields.at("data-sr"),"data",";High #Deltam Validation Region;Events");
  ...
  auto plot = [&](const vector<TH1*> &vpred, const vector<TGraphAsymmErrors*> &vgraphs,TString outputBase) {
    ...
    //  auto hmc = convertToHist(s.yields.at(sname), sname, ";High #Deltam Validation Region;Events");
...
//    leg->SetNColumns(2);
  ...
  plot(altpred, altgraphs, "std_pred_trad_HM_2016");
